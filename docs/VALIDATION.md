...
| Setup consent/model use, JSONC-preserving global registration, ownership drift, and readiness diagnostics | `tests/config.test.ts`, `tests/setup.test.ts`, `tests/registration.test.ts`, `tests/diagnostics.test.ts` |
